...
public class LstmHelper {
    ...
    // Résultat d'un backtest multi-stratégies
    public static class StrategyBacktestResult {
        public String swingTradeType;
        public double stopLossPct;
        public double takeProfitPct;
        public RiskResult result;
        public StrategyBacktestResult(String swingTradeType, double stopLossPct, double takeProfitPct, RiskResult result) {
            this.swingTradeType = swingTradeType;
    ...
    /**
     * Backtest multi-stratégies et multi-paramètres de risk management.
     * Retourne la liste des résultats pour chaque stratégie et chaque combinaison stopLoss/takeProfit.
     */
    public java.util.List<StrategyBacktestResult> backtestMultiStrategies(
            String symbol,
            double initialCapital,
            double riskPerTrade,
            java.util.List<Double> stopLossPcts,
            java.util.List<Double> takeProfitPcts,
            java.util.List<String> swingTradeTypes
    ) throws IOException {
        java.util.List<StrategyBacktestResult> results = new java.util.ArrayList<>();
        LstmConfig baseConfig = lstmTuningService.hyperparamsRepository.loadHyperparams(symbol);
        if (baseConfig == null) {
            logger.info("Aucun hyperparamètre LSTM pour {}.", symbol);
            return results;
        }
        MultiLayerNetwork model = lstmTradePredictor.loadModelFromDb(symbol, jdbcTemplate);
        BarSeries series = getBarBySymbol(symbol, null);
        for (String swingType : swingTradeTypes) {
            for (double stopLossPct : stopLossPcts) {
                for (double takeProfitPct : takeProfitPcts) {
                    // Cloner la config et adapter le type de stratégie
                    LstmConfig config = new LstmConfig();
                    // Copier les paramètres principaux
                    config.setWindowSize(baseConfig.getWindowSize());
                    config.setLstmNeurons(baseConfig.getLstmNeurons());
                    config.setDropoutRate(baseConfig.getDropoutRate());
                    config.setLearningRate(baseConfig.getLearningRate());
                    config.setNumEpochs(baseConfig.getNumEpochs());
                    config.setPatience(baseConfig.getPatience());
                    config.setMinDelta(baseConfig.getMinDelta());
                    config.setKFolds(baseConfig.getKFolds());
                    config.setOptimizer(baseConfig.getOptimizer());
                    config.setL1(baseConfig.getL1());
                    config.setL2(baseConfig.getL2());
                    config.setNormalizationScope(baseConfig.getNormalizationScope());
                    config.setNormalizationMethod(baseConfig.getNormalizationMethod());
                    config.setFeatures(baseConfig.getFeatures());
                    config.setNumLstmLayers(baseConfig.getNumLstmLayers());
                    config.setBidirectional(baseConfig.isBidirectional());
                    config.setAttention(baseConfig.isAttention());
                    ...
                    // Clôture de la dernière position
                    if (inPosition) {
                        double price = series.getBar(series.getEndIndex()).getClosePrice().doubleValue();
                        double pnl = positionSize * ((price - entryPrice) / entryPrice);
                        capital += pnl;
                    ...
                    RiskResult riskResult = RiskResult.builder()
                            ...
                            .stabilityScore(stabilityScore)
                            .build();
                    results.add(new StrategyBacktestResult(swingType, stopLossPct, takeProfitPct, riskResult));
                }
            }
        }
        return results;
