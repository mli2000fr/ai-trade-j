...
public class ChatGptService {
     ...
     */
    public AgentResponse askChatGpt(String prompt) {
        ...
        try{
            ...
            // Sauvegarde en base
            AgentEntity gpt = new AgentEntity(Agent.GPT.getName(), LocalDateTime.now(), prompt, res);
            gptRepository.save(gpt);
            return new AgentResponse(gpt.getId(), res, null);
        } catch (RestClientException e) {
            return new AgentResponse(null, null, "Erreur d'appel à l'API OpenAI : " + e.getMessage());
        }
    }
