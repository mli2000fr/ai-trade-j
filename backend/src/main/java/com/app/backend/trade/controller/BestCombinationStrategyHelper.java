...
public class BestCombinationStrategyHelper {
    ...
    public List<String> getSymbolFitredFromTabSingle(String sort) {
        ...
        String sql = "select symbol from trade_ai.best_in_out_single_strategy s where s.avg_pnl > 0 AND s.profit_factor > 1 AND s.win_rate > 0.5 AND s.max_drawdown < 0.2 AND s.sharpe_ratio > 1 AND s.rendement > 0.05";
